...
namespace AppGet.Infrastructure.Composition
{
    public static class AutofacBuilder
    {
        ...
        static AutofacBuilder()
        {
            ...
            var ass = files.Select(Assembly.LoadFile).ToArray();

            builder.RegisterAssemblyTypes(ass).AsImplementedInterfaces().AsSelf().As(c => c.BaseType).SingleInstance();
            ...
            Container.Resolve<IEventHub>().Publish(new ApplicationStartingEvent(Container));
